...
class TestOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    float r = 0.0;
    r = getScreenRGB(screen_buff, w ,h);
...
private:
  ...
  void act(int action){
    ...
    if (cam_heading_>84)
       cam_heading_=-84;
    else if (cam_heading_<-84)
       cam_heading_=84;
  }

  float getScreenRGB(std::vector<unsigned char> &buf, int w, int h){
    float reward = 0.0;
    cv::Mat image = cv::Mat::zeros( h, w, CV_8UC3 );

    //draw line
    int draw_line_at = 0;
    if (cam_heading_>=-42 && cam_heading_<42){
      draw_line_at = 42 - cam_heading_ - 1;

      int thickness = 2;
      int lineType = cv::LINE_8;
      cv::line( image, cv::Point(draw_line_at,0), cv::Point(draw_line_at,h-1),
    ...
    buf.assign(image.begin<unsigned char>(), image.end<unsigned char>());
    return reward;
  }
